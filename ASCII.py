...
class ASCII_Artwork:
    ...
    def import_adjust(self, path: str, ideal_width: int = 100, height_scale_factor: float = 0.6):
        
        # import image, then greyscale and resize
        img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        scale_factor = ideal_width / img.shape[1]
        scaled_dimensions = (int(img.shape[1] * scale_factor), int(img.shape[0] * scale_factor * height_scale_factor))
